...
```{r F5-fig1, fig.height = 3.5, fig.width = 8, fig.cap = "(left) f0 plotted against perceived height for each token. Horizontal line is the mean of the group means. (right) Same as right panel except each group gets its own horizontal line. Groups are boys (yellow), girls (green), women (blue), and men (red).", echo = FALSE}
...
```{r F5-52, fig.height=3.5, fig.width=8, fig.cap = "(left) A normal distribution fixed along a horizontal line. (right) A Normal distribution allowed to slide along a diagonal line.",echo=FALSE}
...
```
```{r, include = FALSE}
single_line_model = readRDS ('../../models/5_single_line_model.RDS')
```
...
```{r F5-53, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap = "(left) Points and best-fit line. (right) A zoomed-out view of the left panel shows the line intercept at y = 588.7 Hz."}
...
```
```{r, include = FALSE}
single_line_centered_model = readRDS ('../../models/5_single_line_centered_model.RDS')
```
...
```{r F5-slope-n-centered, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap="(left) Best fit line for points predicted by perceived height. Horizontal and vertical lines indicated y and x variable means respectively. (right) Same as left but with a centered predictor. "}
...
```{r, include = FALSE}
group_single_slope_model = 
  readRDS ('../../models/5_group_single_slope_model.RDS')
```
...
```
```{r, include = FALSE}
group_intercepts_model = readRDS ('../../models/5_group_intercepts_model.RDS')
```
...
```{r F5-55, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap="(left) Lines for each distribution in our no-slope model. (right) Lines for each distribution in our shared-slope model. Lines correspond to boys (yellow), girls (green), men (red), and women (blue)."}
...
```{r, include = FALSE}
group_multi_slope_model = 
  readRDS ('../../models/5_group_multi_slope_model.RDS')
```
...
```{r F5-57, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap = "(left) Group-specific intercepts. (middle) Group-specific slopes. (right) Lines for each group: boys (yellow), girls (green), men (red), and women (blue)."}
...
```{r F5-group-slopes, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap = "(left) Group-specific intercepts, slope is zero. (middle) Group-specific intercepts, slope is non-zero but fixed across groups. (right) Group-specific intercepts and group-specific slopes. In each panel, lines correspond to boys (yellow), girls (green), men (red), and women (blue)."}
