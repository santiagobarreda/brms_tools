...
The figure below shows the designs presented in Chapter 4, and indicates which random effects may be estimated for each design (i.e., those crossed with subject).  
...
<p class="caption">(\#fig:F5-designfig2)Data from four groups of subjects (S) divided according to a two grouping factors A and B. Estimable random effects indicated by check marks.</p>
</div>
