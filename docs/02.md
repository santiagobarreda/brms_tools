...
```
## SHA-1 hash of file is a664e0a3a48ec9bd7f98f77fd751bed2e533dce5
```
...
<img src="02_files/figure-html/posteriorplot1-1.png" width="768" />
...
One way to think about what all these numbers mean is to simulate data that has the same characteristics. By doing this and omitting individual components, we can see the influence these have on our data, and what they each represent.  

Below I create effects representing 48 simulated 'female talkers' from a population just like our observed population. Each of these simulated speakers produces 12 productions, and so we need a speakers vector with values that repeat 12 times each to index the alpha_speaker vector. 

I also draw our error, epsilon. It is important to note that this is just 48*12 random draws from this population. There is no distinction between one person's productions and another when it comes to the error. If there were, this would indicate differences in sigma between speakers. 

Finall, I add the Intercept, speaker deflections and random error to make our observed variable. 


```r
## I picked a seed that makes it look like our data. 
set.seed(5)
## this is just the value of our intercept
Intercept = 220.4
## this is a vector of 48 speaker effects
alpha_speaker = rnorm (48, 0, 20)
## this is a vector indicating which speaker produced which utterance
speaker = rep (1:48, each = 12)
## this vector contains the error
epsilon = rnorm (48 * 12, 0, 12)
## the sum of the above components equals our observations
f0_rep = Intercept + alpha_speaker[speaker] + epsilon
```

Below I compare the results of our simulation to our real data. I don't include the code (its at the end of the chapter) so that it is less clear which is the real data and which is not. If you didn't have a clear impression

<img src="02_files/figure-html/unnamed-chunk-20-1.png" width="672" />

Below I make two incomplete types of data: the first contains the intercept and noise only, the second contains the intercept and speaker effects only. 
...
In the figure below, I compare these 'incomplete' datasets to the full simulated data. 


```r
par (mfrow = c(1,3), mar = c(1,2,1,1), oma = c(0,2,0,0))
boxplot (f0_rep_1 ~ speaker, ylim = c(140,320),
         col=c(yellow,coral,deepgreen,teal))
abline (h=220,lty=2)
boxplot (f0_rep_2 ~ speaker, ylim = c(140,320),
         col=c(yellow,coral,deepgreen,teal))
abline (h=220,lty=2)
boxplot (f0_rep ~ speaker, ylim = c(140,320),
         col=c(yellow,coral,deepgreen,teal)  )
abline (h=220,lty=2)
```

<img src="02_files/figure-html/unnamed-chunk-22-1.png" width="672" />



f0_rep_1 = Intercept + alpha_speaker[speaker] + epsilon
f0_rep_2 = Intercept + alpha_speaker[speaker] + epsilon

par (mfrow = c(1,2), mar = c(4,4,1,1))
boxplot (f0_rep_1 ~ speaker, ylim = c(140,320))
boxplot (f0_rep_2 ~ speaker, data = w, ylim = c(140,320))
...
Before fitting a model across multiple cores, you should confirm how many you have. You can use the following commend (you may need to install the `parallel` package):
...
<img src="02_files/figure-html/unnamed-chunk-34-1.png" width="768" />
...
<img src="02_files/figure-html/unnamed-chunk-36-1.png" width="768" />


## Traditionalist corner

Here I am going to compare the output of `brms` to the output of more traditional models. The idea is simply to make a comparison to see how the models we are learning about provide you the same information (and more!) than traditional models. I am not going to talk about the traditional models in any detail. The focus of this section is simply to highlight the potential similarities between different approaches, and to point out where to find this information. This will mostly be useful to people familiar with the traditional approaches! 

### One sample t-test

Below we find the speaker averages and use these to calculate a one-sample t-test. We need to find the speaker averages because the one-sample t test is not a repeated measures design. 


```r
## find speaker average f0
speaker_means = aggregate (f0 ~ speaker, data = w, FUN = mean) 
...
Above we see the output of t-test in R. Below, we see how this information can be recovered from our model. The mean and 95% confidence intervals plainly correspond to our mean and 95% credible intervals. They are mathematically very similar though they are philosophically different. 

[The t statistic is just the mean of the group, divided by the standard deviation of the estimate of the mean](https://en.wikipedia.org/wiki/T-statistic). We can see that if we divide the mean of our posterior mean estimate by the standard deviation of the posterior distribution, we get a value very similar to our t statistic. 



```r
fixed_effect = brms::fixef (multilevel_priors )
fixed_effect
```

```
##           Estimate Est.Error     Q2.5    Q97.5
## Intercept 220.4228  2.966625 214.4374 226.3801
```

```r
## recover the t statistic
fixed_effect[,1] / fixed_effect[,2]
```
...
One thing that is not directly available is a p-value. This is because Bayesian modeling focuses on estimating the values of effects, and the uncertainty in these estimates. As a result, we are not so much focused on the probability that a hypothesis is "true" or "false", but instead on what our models tell us about the relationships in our data more generally. However, you can kind of get a "Bayesian p value" by finding the proportion of samples that are above/below 0. Again, although philosophically there are differences, mathematically this value will roughly correspond to a one-tailed p value for a parameter given data and a model structure. 

Although there are broad similarities, our multilevel model also gives us much, much more information than a t-test. It also does not require us to average within speaker, and can model within speaker and between speaker variation at the same time. That being said, if all you really want is to quickly determine if two groups are likely to be different or not you may only want to use a t-test.

### Lmer

Here I am going to compare the output of `brms` to the output of the `lmer` ("linear mixed-effects regression") function, a very popular function for fitting multilevel models in the lme4 R package. 
...
model = readRDS ('../../models/2_model.RDS')
## inspect model
model
## sample mean
mean (f0)
...
multilevel_model = readRDS ("2_multilevel_model.RDS")
## load downloaded model from working directory
multilevel_model = readRDS ('../../models/2_multilevel_model.RDS')
## inspect model
multilevel_model
## find mean f0 for each speaker
speaker_means = aggregate (f0 ~ speaker, data = w, FUN = mean) 
...
## and load after placing in working directory
multilevel_multicore = readRDS ('2_multilevel_multicore.RDS')
multilevel_multicore = readRDS ('../../models/2_multilevel_multicore.RDS')
## inspect model
multilevel_multicore
## Fit the model yourself, or
set.seed (1)
...
## and load after placing in working directory
multilevel_thinned = readRDS ('2_multilevel_thinned.RDS')
multilevel_thinned = readRDS ('../../models/2_multilevel_thinned.RDS')
## inspect model
multilevel_thinned
mean(f0)
sd(f0)
...
## and load after placing in working directory
multilevel_priors = readRDS ('2_multilevel_priors.RDS')
multilevel_priors = readRDS ('../../models/2_multilevel_priors.RDS')
## inspect model
multilevel_priors
# get t density
x1 = seq (-4,4,.1)
